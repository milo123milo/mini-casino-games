...
uint8_t Touch_Init(void) {
  Wire.begin(CST328_SDA_PIN, CST328_SCL_PIN, I2C_MASTER_FREQ_HZ);
  ...
  attachInterrupt(CST328_INT_PIN, Touch_CST328_ISR, interrupt); // 关联触控中断函数
...
bool I2C_Read(uint8_t Driver_addr, uint16_t Reg_addr, uint8_t *Reg_data, uint32_t Length)
{
  ...
  Wire.write((uint8_t)(Reg_addr >> 8)); // 高8位地址
  Wire.write((uint8_t)Reg_addr);         // 低8位地址
